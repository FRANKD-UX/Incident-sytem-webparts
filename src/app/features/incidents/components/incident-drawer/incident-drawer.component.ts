...
export class IncidentDrawerComponent implements OnInit, OnChanges {
  ...
  get activeStep(): any | null {
    const incident = this.incident as any;
    return incident?.chain?.find((step: any) => step.isCurrentStep) ?? null;
  }

  get checklistItems(): any[] {
    return this.activeStep?.checklistItems ?? [];
  }
